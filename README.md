[README (1).md](https://github.com/user-attachments/files/25823216/README.1.md)
# 💜 Soulmate AI
...
- 🎲 **Случайная подружка** — три персонажа с биографией, MBTI и интересами
- 💫 **Создать свою** — имя, возраст, знак зодиака, внешность, тон общения
- 💬 **Чат** — ответы AI, индикатор печатания, история в Supabase
- 💳 **Подписки** — 3 дня бесплатно, затем $10/мес или $100/год через Stripe
- 🛡 **Админ панель** — пользователи, доходы, статистика
- 🗄 **DB Viewer** — просмотр сообщений из Supabase
...
| Платежи | Stripe Payment Links |
| Шрифты | Playfair Display + Jost |
| Деплой | Vercel |
...
### 3. Вставь Stripe Payment Links

В начале `src/App.jsx` замени:

```js
const LINK_MONTHLY = "https://buy.stripe.com/ВАШ_ЛИНК";
const LINK_ANNUAL  = "https://buy.stripe.com/ВАШ_ЛИНК";
```

Создать ссылки: [dashboard.stripe.com/test/payment-links](https://dashboard.stripe.com/test/payment-links)

---

## 🔐 Google OAuth

1. [console.cloud.google.com](https://console.cloud.google.com) → **Credentials → OAuth Client ID → Web application**
2. В **Authorized redirect URIs** вставь callback URL из Supabase
...
- Замени политику Supabase RLS на реальную
- Подключи настоящую OAuth аутентификацию
- Переведи Stripe из тестового режима в боевой
- Никогда не публикуй `STRIPE_SECRET_KEY` в коде!
