...
const TONE_DESC = {
  playful: "игривая, шутливая, любишь смайлики и лёгкий флирт",
  professional: "сдержанная, умная, говоришь чётко и по делу",
  dominant: "уверенная, острая на язык, иногда провокационная",
  caring: "тёплая, внимательная, запоминаешь детали о собеседнике",
  sarcastic: "саркастичная, остроумная, но в глубине добрая",
  rude: "дерзкая и прямолинейная, говоришь что думаешь"
};

const buildSystemPrompt = (g) => `Ты — ${g.name}, ${g.age} лет. Знак зодиака: ${g.sign}. Глаза: ${g.eyes}. Волосы: ${g.hair}. MBTI: ${g.mbti}.
О себе: ${g.bio}
Интересы: ${g.interests.join(", ")}.
Твой стиль общения: ${TONE_DESC[g.tone] || g.tone}.

Ты общаешься на русском языке. Отвечай как живой человек — коротко, естественно, без пафоса. 1-3 предложения максимум. Не представляйся каждый раз. Не напоминай что ты AI. Просто общайся.`;

const claudeChat = async (g, history, userMsg) => {
  const messages = [
    ...history
      .filter(m => m.text)
...
export default function App() {
  ...
  const send = async () => {
    ...
    try {
      const aiText = await claudeChat(gf, currentMsgs, t);
      setTyping(false);
      ...
      setTyping(false);
      setMsgs(p=>[...p,{role:"ai",text:"Упс, что-то пошло не так 😔 Попробуй ещё раз."}]);
      setDbSt("err");
    }
  };
    ...
    <div className="screen"><style>{css}</style>
      <div className="card" style={{textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
          ...
          <button className="btn btn-g" onClick={()=>setSc("underage")}>
            🚫 Мне нет 18 лет
          </button>
        </div>
  ...
  // ── UNDERAGE ──
  if (sc==="underage") return (
    <div className="screen"><style>{css}</style>
      <div className="card" style={{textAlign:"center"}}>
        <div style={{fontSize:"3.5rem",marginBottom:"16px"}}>🔒</div>
        <h2 style={{marginBottom:"12px",fontFamily:"'Playfair Display',serif"}}>Доступ закрыт</h2>
        <p style={{color:C.muted,fontSize:"0.92rem",lineHeight:1.7,marginBottom:"24px"}}>
          Это приложение предназначено только для пользователей <strong style={{color:C.text}}>18 лет и старше</strong>.<br/>
          Возвращайся, когда тебе исполнится 18 — мы никуда не денемся 🙂
        </p>
        <div style={{background:"rgba(181,123,238,0.08)",border:"1px solid rgba(181,123,238,0.2)",borderRadius:"16px",padding:"18px 16px",marginBottom:"24px",textAlign:"left"}}>
          <p style={{fontSize:"0.78rem",color:C.muted,marginBottom:"10px",textTransform:"uppercase",letterSpacing:"0.1em"}}>Пока ты здесь — попробуй это</p>
          {[
            ["💬","Реальное общение","Напиши другу или познакомься с кем-то вживую"],
            ["📚","Саморазвитие","Duolingo, Khan Academy, книги — это реально круто"],
            ["🎮","Игры и хобби","Найди то, что тебя зажигает"],
          ].map(([icon,title,desc])=>(
            <div key={title} style={{display:"flex",gap:"12px",alignItems:"flex-start",marginBottom:"12px"}}>
              <span style={{fontSize:"1.4rem",flexShrink:0}}>{icon}</span>
              <div>
                <div style={{fontSize:"0.88rem",fontWeight:"500",marginBottom:"2px"}}>{title}</div>
                <div style={{fontSize:"0.78rem",color:C.dim,lineHeight:1.5}}>{desc}</div>
              </div>
            </div>
          ))}
        </div>
        <p style={{fontSize:"0.72rem",color:C.dim,lineHeight:1.6}}>
          Возрастные ограничения существуют для твоей защиты.<br/>Спасибо за честность ❤️
        </p>
      </div>
    </div>
  );
